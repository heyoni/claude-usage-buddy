...
퍼센트는 Claude Code 의 `/usage` 와 **같은 값**입니다. Claude Code 에 로그인돼 있으면 그 계정으로 서버에서 받아옵니다.
오프라인이거나 로그인이 풀리면 말풍선에 "추정" 이 붙고 로컬 기록으로 계산합니다 ([자세히](#사용량은-어디서-가져오나요)).
...
**1. 퍼센트와 초기화 시각 — 서버 값**
Claude Code 는 로그인하면 OAuth 토큰을 macOS 키체인에 넣어 두고, `/usage` 를 치면 그 토큰으로 서버에 물어봅니다.
이 앱도 같은 토큰으로 같은 곳에 물어봅니다. 그래서 말풍선의 퍼센트는 `/usage` 와 같은, **서버가 실제로 세는 숫자**입니다.
7일 한도도 같이 받아와 CLI 와 JSON 에 보여줍니다.

토큰은 읽기만 하고 Anthropic 서버 외에는 어디에도 보내지 않습니다. 갱신은 Claude Code 에 맡깁니다 —
토큰이 만료됐으면 Claude Code 를 한 번 켜면 다시 채워집니다.

이 주소는 공개 문서에 없는 내부 API 라 언제든 바뀔 수 있습니다. 실패하면 아래 로컬 추정으로 자동으로 넘어가고,
말풍선에 "추정" 이 붙습니다. 아예 끄려면 설정에서 `use_server_usage` 를 `false` 로.

**2. 토큰 수와 금액 — 로컬 기록**
Claude Code 는 요청마다 `~/.claude/projects/**/*.jsonl` 에 모델명·토큰 수·시각을 남깁니다.
CLI 의 토큰 수, 오늘·7일 합계, 모델별 비중은 이 파일에서 셉니다.

**금액은 API 정가 환산 추정치입니다.**
...
#### 눈금 맞추기 (서버 값을 못 받을 때만)

서버 값을 못 받으면 퍼센트를 로컬 기록으로 추정하는데, 이때 100% 기준은
*지금까지 가장 많이 쓴 5시간 블록* 입니다. 실제 한도와 다를 수 있으므로,
Claude Code 에서 `/usage` 로 확인한 값을 알려 주면 기준을 역산해 저장합니다.
...
서버 값이 정상적으로 오는 동안에는 이 설정이 쓰이지 않습니다.
...
$ claude-usage-buddy --json
{
  ...
  "week":  { "cost_usd": 96.30, "tokens": 112400000,
             "percent": 18.0, "resets_at": 1789531200 },
  "source": "server",
...
```jsonc
{
  ...
  "use_server_usage": true,     // Claude Code 계정으로 서버 사용률을 받아옴. false 면 로컬 추정만
...
├── single.py     중복 실행 방지
├── remote.py     Claude Code 계정으로 서버 사용률 조회
...
- 서버 사용률은 공개되지 않은 내부 API 로 받아옵니다. Anthropic 이 바꾸면 로컬 추정으로 떨어집니다
  (말풍선에 "추정" 표시).
- 알림은 `osascript` 를 씁니다.
